          ...
          {pricingData.map((tier) => (
            <div 
              ...
              className={`relative p-8 rounded-3xl border ${
                tier.recommended 
                ? 'bg-zinc-900 border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.15)] scale-105 z-10' 
                : 'bg-zinc-900/50 border-zinc-800 hover:border-zinc-700'
              } transition-all duration-300`}
            >
              {tier.recommended && (
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-green-500 text-black font-bold px-4 py-1 rounded-full text-sm uppercase tracking-wider">
                  Bestseller
                </div>
              )}
